...
namespace TeduCoreApp.Application.Implementation
{
    public class BillService : IBillService
    {
        ...
        public PagedResult<BillViewModel> GetPagingForUser(string startDate, string endDate, string keyword
        , int pageIndex, int pageSize, string idUser)
        {
            var query = _orderRepository.FindAll(x => x.CustomerId.ToString() == idUser.ToString());
            if (!string.IsNullOrEmpty(startDate))
            {
                DateTime start = DateTime.ParseExact(startDate, "dd/MM/yyyy", CultureInfo.GetCultureInfo("vi-VN"));
                query = query.Where(x => x.DateCreated >= start);
        ...
        public BillViewModel GetDetail(int billId)
        {
            ...
            //List BillDetail follow to Bill Id
